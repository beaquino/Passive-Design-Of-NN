...
def clip_eta(eta, norm, eps):
    ...
    axis = tuple(range(1, len(eta.shape)))
    ...
    if norm == np.inf:
        ...
        norm = torch.sqrt(torch.maximum(avoid_zero_div, torch.sum(torch.square(eta), axis=axis, keepdims=True)))
        # We must *clip* to within the norm ball, not *normalize* onto the surface of the ball
        factor = torch.minimum(torch.tensor(1.0), torch.divide(torch.tensor(eps), norm))
        eta = eta * factor
    return eta
